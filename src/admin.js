...
export async function admin(request, env) {
  ...
  try {
    return new Response(await renderDashboard(new URL(request.url), env.REPORTS_DB), { headers });
  } catch {
...
export async function renderDashboard(url, db) {
